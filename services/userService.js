...
const { buildMetadata, buildViewUserResponse } = require('../models/dto')
...
class UserService {
    static async getUserList(req) {
        try {
            ...
            return {
                data: buildViewUserResponse(result.users),
                metadata: buildMetadata(req.page, req.pageSize, result.count)
            }
        } catch (err) {
            throw err;
        }
